...
const useStyles = createUseStyles(theme => ({
  button: {
    ...
    color: props => {
      ...
      if (props.textColor) {
        if (theme.palette?.[props.textColor]) return theme.palette?.[props.textColor].main;
    ...
    fontSize: props => {
      ...
      if (props.size) return `${props.size + 3}px`;
      return '14px';
    },
    border: props => {
      ...
      if (props.variant === 'dashed') {
        if (theme.palette?.[props.color]) return `1px dashed ${theme.palette?.[props.color].main}`;
    ...
    '&:disabled': {
      ...
      border: props => {
        if (props.variant === 'outlined') return '1px solid rgba(0, 0, 0, 0.12)';
        if (props.variant === 'dashed') return '1px dashed rgba(0, 0, 0, 0.32)';
        return 'none';
      },
      boxShadow: 'none',
      cursor: 'default',
...
const CCButton = forwardRef((props, ref) => {
  ...
  return (
    <button
      ...
      {...others}
    >
      {isRippling && <span className={classes.ripple} style={{ left: coords.x, top: coords.y }} />}
